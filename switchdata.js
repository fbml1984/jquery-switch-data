        ...
        return this.each(function (i, $dataFrom) {
                    ...
                    $.when(settings.receiveFrom($(this).val())).done(function (response) {
                        if (response.length || response !== null) {
                            ...
                            $.each(response, function (key, value) {
                                items.push(_createLIFromKeyAndValue(value, key, 'destiny'));
                            });
                            $.when($leftUL.html(items)).done(function () {
                                _checkDuplicatedValues();
                            });
                        }
                    }).fail(function (error) {
                        $leftUL.html("");
            ...
            if (settings.search && settings.search.url !== undefined && settings.search.url != null) {
                ...
                $('[name="q"]', $parent).on('keyup', function () {
                    _fireSearch(this.value);
                });
            }

            var $leftUL = $('<ul/>').attr('id', $selectLeft.attr('name'));
            $selectLeft.parent().append($leftUL);

            var $rightUL = $('<ul/>').attr('id', $selectRight.attr('name'));
            $selectRight.parent().append($rightUL);

            if (typeof $optionsSelectRight !== 'undefined' &&
                $optionsSelectRight !== undefined &&
                $optionsSelectRight !== null &&
                $optionsSelectRight.length > 0
            ) {
                var rightListLI = [];
                $.each($optionsSelectRight, function (i, option) {
                    $(option).attr('selected', true);
                    var $rightLI = _createLIFromOption(option, 'origin');
                    rightListLI.push($rightLI);
                });
                $($rightUL).html(rightListLI);
            }
            ...
            $btnMoveToLeft.on('click', function () {
                ...
                $('li', $rightUL).each(function (i, rightLI) {
                    ...
                    if (_append) {
                        _createLIFromLI(rightLI, 'destiny').appendTo($leftUL);
            ...
            $btnMoveToRight.on('click', function () {
                ...
                $.each($('li', $leftUL), function (i, leftLI) {
                    ...
                    if (_append) {
                        _createLIFromLI(leftLI, 'origin').appendTo($rightUL);
            ...
            function _checkDuplicatedValues() {
                var countLeftLI = $('li', $leftUL).length;
                $('li', $rightUL).each(function (i, rightLI) {
                    var rightID = $(rightLI).attr('id');
                    if (countLeftLI) {
            ...
            function _move(item, direction) {
                let id = $(item).attr('id');
                let text = $(item).text();
                if (direction === 'destiny') {
                    var exists = $('option[value="' + id + '"]', $selectRight).length;
                    if (exists) {
                        return false;
                    }
                    $rightUL.append(_createLIFromKeyAndValue(id, text, 'origin'));
                    $selectRight.append(_createOptionFromKeyAndValue(id, text, true));
                    $(item, $leftUL).remove();
                } else if (direction === 'origin') {
                    var exists = $('li[id="' + id + '"]', $leftUL).length;
                    if (exists) {
                        return false;
                    }
                    $leftUL.append(_createLIFromKeyAndValue(id, text, 'destiny'));
                ...
                $.ajax({
                    ...
                    if (response.length) {
                        ...
                        for (var i in response) {
                            items.push(_createLIFromKeyAndValue(response[i][options.search.resultPair.key], response[i][options.search.resultPair.value], 'destiny'));
                        }
                        $.when($leftUL.html(items)).done(function () {
                            _checkDuplicatedValues();
                        });
            ...
            function _createLIFromKeyAndValue(id, text, direction) {
                return $('<li/>')
                    .attr('id', id)
                    .html(text)
                    .on('click', function () {
            ...
            function _createLIFromLI(li, direction) {
                return $('<li/>')
                    .attr('id', $(li).attr('id'))
                    .html($(li).text())
                    .on('click', function () {
            ...
            function _createLIFromOption(option, direction) {
                return $('<li/>')
                    .attr('id', $(option).val())
                    .html($(option).text())
                    .on('click', function () {
